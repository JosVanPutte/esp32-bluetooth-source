        ...
        if(m_msg.sig > 0){
            switch (m_msg.sig) {
                case BT_APP_SIG_WORK_DISPATCH:
                    ...
                    break;
                default:
                    BT_INFO("core %d unhandled sig: %d", xPortGetCoreID(), m_msg.sig);
                    break;
                }
...
void filter_inquiry_scan_result(esp_bt_gap_cb_param_t *param){
    ...
    if (!esp_bt_gap_is_valid_cod(cod)) {
        BT_INFO("cod is 0x%x not valid", cod);
        return;
    }
    if (!(esp_bt_gap_get_cod_srvc(cod) & ESP_BT_COD_SRVC_RENDERING)) {
        BT_INFO("cod is 0x%x not a rendering (0x%x)", cod, ESP_BT_COD_SRVC_RENDERING);
    }
    ...
    if (eir) {
        ...
        if (strcmp((char *)s_peer_bdname,  s_BT_sink_name.c_str()) != 0) {
            BT_INFO("s_peer_bdname %s, s_BT_sink_name %s", s_peer_bdname, s_BT_sink_name.c_str());
        }
...
void bt_app_gap_cb(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t *param){
    ...
    case ESP_BT_GAP_DISC_STATE_CHANGED_EVT: {
        ...
        if (param->disc_st_chg.state == ESP_BT_GAP_DISCOVERY_STOPPED) {
            if (m_a2d_state == APP_AV_STATE_DISCOVERED) {
                ...
                BT_INFO("Device discovery failed, continue to discover...");
                esp_bt_gap_start_discovery(ESP_BT_INQ_MODE_GENERAL_INQUIRY, 10, 0);
            }
        } else if (param->disc_st_chg.state == ESP_BT_GAP_DISCOVERY_STARTED) {
            //BT_INFO("Discovery started.");
        }
        break;
    }
    ...
    case ESP_BT_GAP_AUTH_CMPL_EVT: {
        if (param->auth_cmpl.stat == ESP_BT_STATUS_SUCCESS) {
            BT_INFO("authentication success: %s", param->auth_cmpl.device_name);
        } else {
            BT_INFO("authentication failed, status:%d", param->auth_cmpl.stat);
        }
        break;
    }
    case ESP_BT_GAP_PIN_REQ_EVT: {
        BT_INFO("ESP_BT_GAP_PIN_REQ_EVT min_16_digit:%d", param->pin_req.min_16_digit);
        if (param->pin_req.min_16_digit) {
            BT_INFO("Input pin code: 0000 0000 0000 0000");
            esp_bt_pin_code_t pin_code = {0};
            esp_bt_gap_pin_reply(param->pin_req.bda, true, 16, pin_code);
        } else {
            BT_INFO("Input pin code: 1234");
            ...
            BT_INFO("Input pin code: %s", s_pin_code);
            esp_bt_gap_pin_reply(param->pin_req.bda, true, s_pin_code_length, s_pin_code);
        }
        break;
    }
    case ESP_BT_GAP_CONFIG_EIR_DATA_EVT:
        BT_INFO("ESP_BT_GAP_CONFIG_EIR_DATA_EVT: stat:%d num:%d", param->config_eir_data.stat, param->config_eir_data.eir_type_num);
        break;

    case ESP_BT_GAP_MODE_CHG_EVT:
        BT_INFO("ESP_BT_GAP_MODE_CHG_EVT: mode: %d", param->mode_chg.mode);
        break;


    default: {
        BT_INFO("core %d unhandled event: %d", xPortGetCoreID(), event);
        break;
    }
    }
    return;
...
void bt_app_av_sm_hdlr(uint16_t event, void *param){
    ...
    case APP_AV_STATE_DISCONNECTING:
        ...
        break;
    default:
        BT_INFO("invalid state %d", m_a2d_state);
        break;
    }
}
//---------------------------------------------------------------------------------------------------------------------
...
void bt_app_av_state_connecting(uint16_t event, void *param){
    ...
    case BT_APP_HEART_BEAT_EVT:
        a2d = (esp_a2d_cb_param_t *)(param);
        if(a2d) if (a2d->conn_stat.state == ESP_A2D_CONNECTION_STATE_CONNECTED)  BT_INFO("a2dp connected");
    ...
    switch (m_media_state) {
        case APP_AV_MEDIA_STATE_IDLE: {
            if (event == BT_APP_HEART_BEAT_EVT) {
                //BT_INFO("a2dp media ready checking ...");
                esp_a2d_media_ctrl(ESP_A2D_MEDIA_CTRL_CHECK_SRC_RDY);
            } else if (event == ESP_A2D_MEDIA_CTRL_ACK_EVT) {
                if (a2d->media_ctrl_stat.cmd == ESP_A2D_MEDIA_CTRL_CHECK_SRC_RDY &&
                        a2d->media_ctrl_stat.status == ESP_A2D_MEDIA_CTRL_ACK_SUCCESS) {
                    //BT_INFO("a2dp media ready, starting ...");
        ...
        case APP_AV_MEDIA_STATE_STARTING: {
            if (event == ESP_A2D_MEDIA_CTRL_ACK_EVT) {
                ...
                if (a2d->media_ctrl_stat.cmd == ESP_A2D_MEDIA_CTRL_START &&
                        a2d->media_ctrl_stat.status == ESP_A2D_MEDIA_CTRL_ACK_SUCCESS) {
                    BT_INFO("a2dp media start successfully.");
                    //s_intv_cnt = 0;
                    m_media_state = APP_AV_MEDIA_STATE_STARTED;
                } else {
                    // not started succesfully, transfer to idle state
                    BT_INFO("a2dp media start failed.\n");
                    m_media_state = APP_AV_MEDIA_STATE_IDLE;
                }
            }
            break;
...
bool a2dp_source_init(String deviceName, String pinCode){
    ...
    if(!btStart()) {BT_INFO("Failed to initialize controller"); return false;}
    else BT_INFO("controller initialized");

    res = esp_bluedroid_init(); // init bluedroid
    if(res != ESP_OK){BT_INFO("initialize bluedroid failed"); return false;}

    res = esp_bluedroid_enable(); // activate bluedroid
    if(res != ESP_OK){BT_INFO("enable bluedroid failed"); return false;}
    m_bt_enabled = true;
...
int32_t bt_app_a2d_data_cb(uint8_t *data, int32_t len){ // BT data event
    ...
    static uint8_t i = 0;
    if(sampleRate != 44100) BT_INFO("SampleRate must be 44100, not %d", sampleRate);
